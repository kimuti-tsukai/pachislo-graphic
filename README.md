...
```typescript
import { Game, Config } from "https://raw.githubusercontent.com/username/repo/main/src/pachislo.ts";
```
...
interface UninitializedState { type: "Uninitialized" }
interface NormalState { type: "Normal"; balls: number }
interface RushState { type: "Rush"; balls: number; rushBalls: number; n: number }
```
...
interface WinResult { type: "Win"; winType: Win }
interface LoseResult { type: "Lose"; loseType: Lose }

enum Win { Default = "Default", FakeWin = "FakeWin" }
enum Lose { Default = "Default", FakeLose = "FakeLose" }
```
...
abstract class UserOutput {
  ...
  abstract lotteryRushContinue(result: LotteryResultType, slot: SlotOutput): void
}
```
